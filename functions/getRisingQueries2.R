...
#' \itemize{
#'   \item \code{risingSearches}: The rising related search queries.
...
getRisingQueries2 <- function(terms, geo=NULL, property=NULL, category=NULL, startDate=NULL, endDate=NULL, api.key) {
  ...
  if(httr::http_error(f) == T){
    ...
    if(length(d)==0){
      ...
      names(res)<-c("risingSearches", "value")
